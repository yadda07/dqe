-- FUNCTION: rip_avg_nge.dqe2(text, text)
...
BEGIN
    ...
    cables_data AS (
        ...
        FROM (
            SELECT 
                ...
                length AS longueur,
                gid_dc2
            FROM rip_avg_nge.fddcpi2(p_sro)
    ...
    all_rows AS (
        ...
        SELECT 1 AS ordre, 'Nbre de Prises DTR' AS designation, 'prise' AS unite, 
            CASE WHEN p_type = 'T' THEN 0 ELSE 
                COALESCE((SELECT SUM(CASE WHEN phase = 'DTR' THEN noe_nblr ELSE 0 END) 
                    FROM rbal.rbal_auvergne WHERE sro ILIKE '%' || p_sro || '%'), 0) 
            END AS quantite,
            CASE WHEN p_type = 'T' THEN NULL ELSE
                (SELECT string_agg(gid::text, ',') FROM rbal.rbal_auvergne 
                WHERE sro ILIKE '%' || p_sro || '%' AND phase = 'DTR' LIMIT 1)
            END AS gids
        ...
        SELECT 2 AS ordre, 'Nbre de Prises RAD' AS designation, 'prise' AS unite, 
            ...
            CASE WHEN p_type = 'T' THEN NULL ELSE
                (SELECT string_agg(gid::text, ',') FROM rbal.rbal_auvergne 
                WHERE sro ILIKE '%' || p_sro || '%' AND phase IN ('RAD', 'RAD_H_DCE') LIMIT 1)
            END AS gids
        ...
        SELECT 3 AS ordre, 'SRO' AS designation, 'U' AS unite, 
            CASE WHEN p_type = 'T' THEN 0 ELSE 
                COALESCE((SELECT COUNT(DISTINCT sro) FROM rbal.rbal_auvergne WHERE sro ILIKE '%' || p_sro || '%'), 0) 
            END AS quantite,
...
ALTER FUNCTION rip_avg_nge.dqe2(text, text)
    OWNER TO ownergrp_auvergne;

GRANT EXECUTE ON FUNCTION rip_avg_nge.dqe2(text, text) TO PUBLIC;

GRANT EXECUTE ON FUNCTION rip_avg_nge.dqe2(text, text) TO auvergne_sch_etudes;

GRANT EXECUTE ON FUNCTION rip_avg_nge.dqe2(text, text) TO ownergrp_auvergne;

GRANT EXECUTE ON FUNCTION rip_avg_nge.dqe2(text, text) TO sdupays;

COMMENT ON FUNCTION rip_avg_nge.dqe2(text, text)
    IS 'Fonction permettant de calculer les quantités et longueurs pour le DQE, avec prise en compte précise du mode de pose (souterrain, aérien, façade, immeuble) grâce à la fonction fddcpi2.';
